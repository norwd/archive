...
int kcomg=0;     /* Keep <-> sign (previously: - ) */
int kcomp=0;     /* Keep <$> sign (previously: = ) and <%> sign */
int kcoma=0;     /* Keep <~> sign */
  ...
  char *argv[];*/
{
    ...
    if (sign == '-' || sign == '+') {
      ...
      if (optn >= '@' && optn <= 'Z') {
        ...
        switch (optn) {
           ...
           case 'p':
             if (val == '0') {
               kcomg=0; kcomp=0; /* Keep <-> , <%> and <$> */
             } else if (val == '1') {
               kcomg=1; kcomp=1; /* <-> , <%> and <$> stripped */
             } else if (val == '2') {
               kcomg=2; kcomp=1; /* <-> converted to space and <%>, <$> stripped */
             }
             break;
           case 'q':
             if (val == '0') {
  ...
  /* Alignment sign <~> deleted in many cases */
  if (kcomt || kcomg || (s_hard > 1)) kcoma = 1;
  return 0;
}
  ...
  if (mute == 0) {
    ...
    switch (kcomp) {
       ...
       case 1: fprintf (stderr,"%s\n","Remove paragraph start / end code");
           break;
       case 2: fprintf (stderr,"%s\n","Remove para codes and add extra newline at end");
           break;
    }
    switch (kcomg) {
    ...
    switch (kcoma) {
       case 0: fprintf (stderr,"%s\n","Keep text misalignment code");
           break;
       case 1: fprintf (stderr,"%s\n","Remove text misalignment code");
           break;
    }
    switch (white) {
       case 0: fprintf (stderr,"%s\n","Keep white space");
    ...
    /* Check if normal paragraph line handling is overruled by locus selection */
    if (invar[0] != ' ' && invar[0] != 'P') toppar = -1;
    if (exvar[0] == 'P') toppar = -1;
    switch (toppar) {
       case -1: fprintf (stderr,"%s\n","Normal paragraph text selection nullified");
           break;
...
char *argv[];*/
{
  ...
  /* For reference: */
  char *what = "@(#)ivtt\t\t1.3\t2023/01/21 RZ\n";

  /* Parse command line options */
  /* Do this first, in order to get the "mute" option first */
  erropt = ParseOpts(argc, argv);
  if (mute == 0) {
    fprintf (stderr,"Intermediate Voynich Transliteration Tool (v 1.3)\n\n");
  }
