...
Vec up(goal.y *left.z - goal.z * left.y, goal.z *left.x - goal.x * left.z,
       goal.x *left.y - goal.y * left.x);

#include <pthread.h>
pthread_t threadIds[h];
Vec img[w * h];
...
void *plotPixel(void *arg) {
  ...
  for (int x = w; x--;) {
    ...
    for (int p = samplesCount; p--;)
      color =
          color + Trace(position, !(goal + left * (x - w / 2 + randomVal()) +
                                    up * (y - h / 2 + randomVal())));
