					...
					struct sentence *cmd)
{
	...
	if(cmd->object == NULL)
		{
		output(player, "You must tell me what to break.\n");
		return FAILURE;
		}
	...
	output(player, "Oh, dear.  Now the %s is broken.\n", cmd->object->name);
					...
					struct sentence *cmd)
{
	...
	if(cmd->object == NULL)
		{
		output(player, "You must tell me what to fix.\n");
		return FAILURE;
		}
	if(!hasattr(cmd->object, "broken"))
		{
		output(player, "The %s is not broken.\n", cmd->object->name);
		return FAILURE;
		}
	if(!contains(player->contents, cmd->xobject))
		{
	...
	output(player, "Somehow you manage to fix the %s.\n", cmd->object->name);
					...
					struct sentence *cmd)
{
	...
	if(cmd->object == NULL)
		{
		output(player, "You must tell me what to cut.\n");
		return FAILURE;
		}
	if(!contains(player->contents, cmd->xobject))
		{
	...
	if(!hasattr(cmd->object, "soft"))
		{
		output(player, "I don't think you can cut the %s with the %s.\n",
	...
	output(player, "The %s is now cut in two.\n", cmd->object->name);
					...
					struct sentence *cmd)
{
	...
	output(player, "You're not quite ready for Carnegie hall,\n");
	output(player, "but you do manage to force out a recognizable tune.\n");
					...
					struct sentence *cmd)
{
	...
	if(cmd->preposition == NULL || strcmp(cmd->preposition, "with") != 0 ||
			cmd->xobject == NULL)
		{
		output(player, "You must tell me what to %s with.\n", cmd->verb);
		return FAILURE;
		}
	if(!hasattr(cmd->object, "lock"))
		{
		output(player, "You can't lock the %s.\n", cmd->object->name);
		return FAILURE;
		}
	if(Isopen(cmd->object))
		{
		output(player, "The %s is open.\n", cmd->object->name);
		return FAILURE;
		}
	if(!contains(player->contents, cmd->xobject))
		{
	...
	if(!hasattr(cmd->xobject, "key"))
		{
		output(player, "The %s won't %s the %s.\n",
	...
	if(strcmp(cmd->verb, "lock") == 0)
		...
		if(hasattr(cmd->object, "locked"))
			{
			output(player, "The %s is already locked.\n", cmd->object->name);
			return FAILURE;
			}

		setattr(cmd->object, "locked");
		output(player, "The %s is now locked.\n", cmd->object->name);
		return SUCCESS;
		}
		...
		if(!hasattr(cmd->object, "locked"))
			{
			output(player, "The %s is already unlocked.\n", cmd->object->name);
			return FAILURE;
			}

		unsetattr(cmd->object, "locked");
		output(player, "The %s is now unlocked.\n", cmd->object->name);
					...
					struct sentence *cmd)
{
	...
	if(cmd->object == NULL)
		{
		output(player, "You must tell me what to %s.\n", cmd->verb);
		return FAILURE;
		}
	if(!hasattr(cmd->object, "lock"))
		{
		output(player, "You can't %s the %s.\n", cmd->verb, cmd->object->name);
		return FAILURE;
		}
	if(Isopen(cmd->object))
		{
					...
					struct sentence *cmd)
{
	...
	output(player, "After spending 20 luxurious minutes in the shower,\n");
	output(player, "singing three full-length show tunes, and using up\n");
	output(player, "all the hot water, you emerge clean and refreshed.\n");
					...
					struct sentence *cmd)
{
	...
	if(objp != cmd->object)
		return CONTINUE;
	output(player, "As you pick it up, the sword briefly glows blue.\n");
	return CONTINUE;
}
					...
					struct sentence *cmd)
{
	...
	if(hazmatsuit == NULL || !contains(player->contents, hazmatsuit))
		{
		output(player, "The container is far too dangerous to pick up.\n");
		return FAILURE;
		}
					...
					struct sentence *cmd)
{
	...
	output(player, "You feel much safer wearing the bright yellow Hazmat suit.\n");
	return SUCCESS;
}
