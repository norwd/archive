	...
	switch(tok)
		...
		case CSETOBJSTATE:
			...
			if(hasattr(cmd->object, attr))
				{
				output(actp, "The %s is already %s.\n",
		...
		case CHKTOOLQUAL:
			...
			if(!hasattr(cmd->xobject, attr))
				{
				output(actp, "You can't %s things with a %s.\n",
		...
		case CHKTOOLSTATE:
			...
			if(!hasattr(cmd->xobject, attr))
				{
				output(actp, "The %s isn't %s enough to %s.\n",
		...
		case CHKNTOOLSTATE:
			...
			if(hasattr(cmd->xobject, attr))
				{
				output(actp, "The %s is too %s to %s.\n",
					cmd->xobject->name, attr, cmd->verb);
				return FAILURE;
				}
			break;
